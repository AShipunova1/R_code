...
```{r save common names and itis in a separate data frame}
## save common names and itis in a separate data frame ----
fhier_common_names <-
...
```{r A function to make a plot by spp.}
## A function to make a plot by spp. ----
...
```{r Add the fhier_common_names we made earlier}
...
### test: For each region sum counts for one species, should be the same as before
```{r test: For each region sum counts for one species}
#| classes: test
...
```{r keep only entries for spp. in the top ten list, separately for each region}
## keep only entries for spp. in the top ten list, separately for each region ----
...
```{r test: For the top 10, for each region sum separately MRIP and FHIER counts for one species}
#| classes: test
#### test: For the top 10, for each region sum separately MRIP and FHIER counts for one species, ----
# should be the same as before
...
### GOM plots for each common name from the top spp.
```{r GOM plots for each common name from the top spp.}
### GOM plots for each common name from the top spp. ----
...
## Count Index: use a MRIP/FHIER count ratio (to keep Y-axes the same across plots)
```{r Count Index: use a MRIP/FHIER count ratio}
...
```{r calculate_cnt_index function}
## calculate_cnt_index function ----
