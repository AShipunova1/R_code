...
```{r most n frequent FHIER species }

get_n_most_frequent_fhier <- function(df_name = NA, n = n, quantity_field_name = quantity_field_name) {
  # a default df
  if (not(is.data.frame(df_name))) {
    df_name <- fhier_quantity_by_species_df
  }
  # if the field name had no quotes - add them
  tryCatch(
  ...
  # sort in descending order and get n first
  df_name %>%
...
```{r combine wave data for fhier and mrip}
