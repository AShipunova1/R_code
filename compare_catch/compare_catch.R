...
plot(mrip_and_fhier$mrip_estimate_catch_by_species, 
     col="blue", ylim=c(0, 20000)) #100000

# Graph fhier with red dashed line and square points
lines(mrip_and_fhier$fhier_quantity_by_species, 
      pch = 22, 
      col="red")
str(mrip_and_fhier_w_names)
# ---- plot catch by species ----
mrip_and_fhier_w_names <- inner_join(scientific_names_w_mrip, mrip_and_fhier, by = "SP_CODE")
my_colors <- c("MRIP" = "blue", "FHIER" = "red")
counts_plot <-
  mrip_and_fhier_w_names %>%
  mutate(SP_CODE = ifelse(SP_CODE >= 7000000000, SP_CODE, NA)) %>%
  mutate(fhier_quantity_by_species = ifelse(fhier_quantity_by_species <= 10000, fhier_quantity_by_species, NA)) %>%
  mutate(mrip_estimate_catch_by_species = ifelse(mrip_estimate_catch_by_species <= 100000, mrip_estimate_catch_by_species, NA)) %>%
  mutate(order = fct_reorder(as.factor(SP_CODE), COMMON_NAME.x)) %>% 
  # str()
  ggplot(aes(x = order,
             y = fhier_quantity_by_species,
             colour = "FHIER"
             ),
         size = 2,
         alpha = 0.1
  ...
  geom_point(aes(x = order,
                   # ,
                 y = mrip_estimate_catch_by_species, 
                 colour = "MRIP"
                ),
             size = 1
             ) +
