...
#### Current file: ~/R_code_github/compare_catch/auxiliary/compare_catch_data_preparation.R
...
### keep only 2022 ---- note: year() is a function from package "lubridate", loaded from "tidyverse"
...
# all dolphin counts for FHIER are combined for further comparison
...
# rename the column as in FHIER for future use
gom_acl_top_spp <-
...
### remove DFs (states) where is no MRIP counts ----
remove_no_mrip_cnts <- function(my_df_list) {
  ...
  for (i in 1:length(my_names)) {
    # for each name of the current DF
    current_name <- my_names[[i]]
    # if there are MRIP counts, save in the new_list with the same name
