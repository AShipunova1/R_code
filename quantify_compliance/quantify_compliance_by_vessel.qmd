...
```{r combine plots}
plot_by_time <- function(my_df, vessel_id, permit_gr, time_period_name) {
  browser()
  my_df %>% 
  filter(permit == permit_gr) %>%
  pivot_longer(starts_with("percent"), names_to = "yes_or_no", values_to = "compliance_percent") %>%
  ...
  labs(title = paste0("compliance for ", vessel_id, " by ", time_period_name),
       x ="quarter", y = "compliance percent")

}
head(percent_by_quarter)
quarter_p <- plot_by_time(percent_by_quarter, "1000042", "gom", "quarter")
month_p <- plot_by_time(percent_by_month, "1000042", "gom", "month")
