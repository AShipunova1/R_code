---
title: "Quantify compliance by vessel"
date: today
output:
  html_document
format:
...
```{r separate vessels non-compliant at least once per year}
# separate vessels non-compliant at least once per year ----
...
glimpse(all_compl_vs_non_compl_per_year_cnt_list)
...
```{r remove non-compl vessels from compliant, to count each vessel once per year}
...
```{r get total unique vessel_ids per year_region}
# get total unique vessel_ids per year_region ----
...
```{r add total vessels_cnt_per_year_reg}
# add total vessels_cnt_per_year_reg ----
...
```{r get perc non_compl vs. total for each year_region}
# get perc non_compl vs. total for each year_region ----
...
# Non compliant only
```{r Non compliant only}
# Non compliant only ----
```

# 1) count percents - a given vsl non_compl per counted weeks total
```{r 1) count percents - a given vsl non_compl per counted weeks total}
# 1) count percents - a given vsl non_compl per counted weeks total ----
```

## 1a) how many weeks each vessel was present
```{r 1a) how many weeks each vessel was present}
## 1a) how many weeks each vessel was present ----
...
```{r 1b) percent of compl/non-compl per total weeks each vsl was present}
## 1b) percent of compl/non-compl per total weeks each vsl was present ----
