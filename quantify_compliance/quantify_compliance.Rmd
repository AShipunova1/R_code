...
```
```{r looking at the proportion of reports submitted vs flag }
glimpse(compl_clean)
# Rows: 167,607
# Columns: 21

```
```{r Separate SA and GOM permits }
```
...
head(compl_clean_sa_vs_gom_plus_dual_0)
```
...
```
```{r}
names(compl_clean_sa_vs_gom_plus_dual) %>% length()
```
```
```{r}
head(compl_clean_sa_vs_gom_plus_dual[20,1])
```
...
```
```{r show the proportion compliant and the proportion non compliant? And show Gulf on a separate plot than SA }
