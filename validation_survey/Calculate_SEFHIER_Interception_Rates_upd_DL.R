...
ptheme<-  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
perms<- perm %>% mutate(vsl_num=`Vessel Official Number`,
...
a_files<- aga %>% select(asg_code,ASG_CODE,ALL_SITE_COMMENTS,REASON1,REASON2,SITEHRS)

## JOIN THE I1 AND LIMITED A DATA
i1a<- i1 %>% left_join(a_files,by=c("asg_code")) %>%
  ## NO TYPE DISCARD OR CATCH RECORDS
  mutate(nodat_ints=case_when((NUM_TYP2==0 & NUM_TYP3==0) ~ 1, TRUE ~ 0),
  ## ALL RECORDS
         comp_ints=1,
  ## IDENTIFY STATE WITH WORDS
        State=case_when(ST=="01" ~ "AL", ST=="12" ~ "FL", ST=="22" ~ "LA",
                        ST=="28" ~ "MS", ST=="48" ~ "TX",
        ...
        is.na(ST)==TRUE & CNTY %in% c(5,15,17,33,71,81,87,91,103,115) ~ "FL",
        is.na(ST)==TRUE & CNTY==75 & INTSITE %in% c(6,232,311) ~"LA",
        is.na(ST)==TRUE & CNTY==315 ~ "TX"),
  ...
  ## DESIGNATE ALL FL TRIPS IN GULF COUNTIES AS GULF TRIPS                        
        State=="FL" & CNTY %in% c(33,113,91,131,5,45,37,129,65,123,29,75,17,53,
                                  101,57,103,81,115,15,71,21) ~ 1,
...
refs<- ref %>% mutate(refs=1,
   ...
   ## IDENTIFY STATE WITH WORDS
     ...
     is.na(ST)==TRUE & CNTY==47 ~ "MS",
     is.na(ST)==TRUE & CNTY==315 ~ "TX",
     is.na(ST)==TRUE & CNTY %in% c(5,15,17,33,71,75,81,87,91,103,115) ~ "FL",
     is.na(ST)==TRUE & CNTY==75 & INTSITE %in% c(6,232,311) ~"LA",
     is.na(ST)==TRUE & CNTY==315 ~ "TX")) 
...
int_cnts_stym<- int_cnts_stym %>% left_join(ref_cnts_stym, 
...
p1<-ggplot(data=int_cnts_stym,aes(x=MONTH,y=tot_comp,group=State,color=State,shape=State)) +
    ...
    theme_bw()+ ptheme 
  ## SAVE GRAPH
    ggsave(p1,file=paste0(out1,"ints_month.png"),width=10,height=7)

## PLOT OF REFUSALS BY MONTH AND STATE
p1<-ggplot(data=int_cnts_stym,aes(x=MONTH,y=tot_refs,group=State,color=State,shape=State)) +
  geom_line(linewidth=2)+
  geom_point(size=4, stroke=2)+
  ...
  theme_bw()+ ptheme
  ## SAVE GRAPH
  ggsave(p1,file=paste0(out1,"refs_month.png"),width=10,height=7)

##############                  DECLARATION DATA                   #############

## ADD MONTH / YEAR TO THE DECLARATION DATA
dec<- dec %>% mutate(MONTH=as.character(substr(as.Date(TRIP_START_DATE,format="%m/%d/%Y"),6,7)),
                     YEAR=as.character(substr(as.Date(TRIP_START_DATE,format="%m/%d/%Y"),1,4)),
...
decs<- decs %>% filter(TRIP_TYPE %in% c("CHARTER","HEADBOAT") & INTENDED_FISHING_FLAG=="YES" & 
            ## FILTER DATA TO INCLUDE ONLY 2022
            (YEAR=="2022")) %>%
  ## DESIGNATE GULF TRIPS (AL, TX, MS, AND LA TRIPS ARE ALWAYS CODED AS GOM)
  mutate(GulfTrip=case_when(State %in% c("AL","TX","MS", "LA") ~ 1,
           ## FL TRIPS THAT INDICATE GOM STAT ZONES CODED AS GULF
           State=="FL" & STAT_ZONE %in% c("003","004","005","006","007","008",
                                          "009","010","018","019","021") ~ 1,
           ## FL TRIPS IN KEYS STAT_ZONE WITH GULF PERMITS ARE CODED AS GULF
           State=="FL" & STAT_ZONE %in% c("001","002","748") & Perm_Group=="GOM" ~ 1,
           TRUE ~ 0),
...
p1<-ggplot(data=dec_cnts_stym,aes(x=MONTH,y=tot_decs,group=State,color=State,shape=State)) +
  geom_line(linewidth=2)+
  geom_point(size=4, stroke=2)+
  ...
  theme_bw()+ ptheme
  ## SAVE GRAPH
    ggsave(p1,file=paste0(out1,"decs_month.png"),width=10,height=7)
...
logb<- logb %>% mutate(MONTH=as.character(substr(as.Date(TRIP_START_DATE,format="%m/%d/%Y"),6,7)),
         YEAR=as.character(substr(as.Date(TRIP_START_DATE,format="%m/%d/%Y"),1,4)),
...
logbs<- logbs %>% ## DESIGNATE GULF TRIPS (AL, TX, MS, AND LA TRIPS ARE ALWAYS CODED AS GOM)
  mutate(GulfTrip=case_when(State %in% c("AL","TX","MS", "LA") ~ 1,
        ## FL TRIPS WITH A GULF COUNTY AS A START COUNTY ARE CODED AS GULF 
          State=="FL" & START_PORT_COUNTY %in% c("ESCAMBIA","SANTA ROSA","OKALOOSA",
        ...
        ## FL TRIPS IN MONROE COUNTY WITH GOM PERMIT GROUP ARE CODED AS GULF
          State=="FL" & START_PORT_COUNTY=="MONROE" & Perm_Group=="GOM" ~ 1,
        ## UNSPECIFIED COUNTIES IN FLORIDA WITH A GOM PERMIT ARE CODED AS GULF
          State=="FL" & START_PORT_COUNTY=="NOT-SPECIFIED" & Perm_Group=="GOM" ~ 1,
          TRUE ~ 0),
...
p1<-ggplot(data=logb_cnts_stym,aes(x=MONTH,y=tot_logs,group=State,color=State,shape=State)) +
  geom_line(linewidth=2)+
  geom_point(size=4, stroke=2)+
  ...
  theme_bw()+ ptheme 
  ## SAVE GRAPH
  ggsave(p1,file=paste0(out1,"logs_month.png"),width=10,height=7)
...
## THE DENOMINATOR)
cnts_stym<- cnts_stym %>% mutate(ref_perc=100*(tot_refs/tot_ints),
...
ys<-c("ref_perc","com_dec_perc","com_log_perc","int_dec_perc","int_log_perc")
ylabs<-c("Refusals (%)",rep("Intercepted Trips (%)",4))
titles<-c("Percent of Refusals (Incomplete/All Interviews)","Interception Rate - Completed Interviews/Declarations",
          "Interception Rate - Completed Interviews/Logbooks","Interception Rate - All Interviews/Declarations",
...
p<- cnts_stym %>% select (MONTH, ys[i],State) %>% mutate(yval=ys[i]) %>%
  filter(!is.na(State))
p1<-ggplot(p,aes_string(x="MONTH",y=ys[i],group="State",color="State",shape="State")) +
  geom_line(linewidth=2)+
  geom_point(size=4, stroke=2)+
  ...
  print(p1)
  ## SAVE GRAPH
  ggsave(p1,file=paste0(out1,"st_",ys[i],".png"),width=10,height=7)
}
...
## VS COMPLETED INTERVIEWS - AS THE NUMERATOR AND DECLARATIONS OR LOGBOOKS AS 
## THE DENOMINATOR)  
cnts_ym<- cnts_ym %>% mutate(ref_perc=100*(tot_refs/tot_ints),
            com_dec_perc=100*(tot_comp/tot_decs),
            com_log_perc=100*(tot_comp/tot_logs),
            int_dec_perc=100*(tot_ints/tot_decs),
            int_log_perc=100*(tot_ints/tot_logs))
...
for ( i in 1:length(ys)) {
  ...
  p1<-ggplot(p,aes_string(x="MON",y=ys[i])) +
    ...
    geom_point(size=4, stroke=2,color="#0570B0")+
    xlab("Month")+ ylab(ylabs[i])+
    ggtitle(titles[i])+
    ...
    print(p1) 
    ## SAVE GRAPH
    ggsave(p1,file=paste0(out1,"ym_",ys[i],".png"),width=10,height=7)
  
}

## SELECT THE COMPLETED INTERVIEW VALUES BY YEAR AND MONTH
tots_comp<- cnts_ym %>% select(YEAR,MONTH,tot_comp) %>% mutate(tots=tot_comp,
    cnt_type="Completed Interviews") %>% select(YEAR,MONTH,tots,cnt_type)
## SELECT THE LOGBOOK VALUES BY YEAR AND MONTH
tots_all<- cnts_ym %>% select(YEAR,MONTH,tot_logs) %>% mutate(tots=tot_logs,
    cnt_type="Total Logbooks") %>% select(YEAR,MONTH,tots,cnt_type)
...
p1<-ggplot(tot_graph, aes(x = MONTH, y = tots))+
  geom_col(position="dodge", stat="identity",aes(fill = cnt_type), width = 0.8) + 
  coord_flip() + 
  ...
  ## SAVE GRAPH
    ggsave(p1,file=paste0(out1,"int_log_bars.png"),width=10,height=7)

## GRAPH THE INTERCEPTION RATE OVER TOP OF HORIZONTAL BARS FOR INTERVIEWS/LOGBOOKS
  p1<-ggplot(tot_graph, aes(x = MONTH, y = tots))+
  geom_col(stat="identity",position="dodge",aes(fill = cnt_type), width = 0.8) +
  # geom_line(data=tot_graph,aes(x=MONTH,y=com_log_perc*1000,group=cnt_type),col="#8E24AA",lty=5,lwd=1.5)+
  geom_line(data=tot_graph,aes(x=MONTH,y=com_log_perc*1000,group=cnt_type),col="#F0AC96",lty=5,lwd=1.5)+
  scale_y_continuous("Number of Interviews / Logbooks",sec.axis = sec_axis(~./1000,name="Interception Rate"))+
  coord_flip() + 
  ylab("Number of Interviews / Logbooks") + xlab("Month") +
  scale_x_discrete(limits=rev)+
  scale_fill_manual("Record Type",values=c("darkslategray1", "#0570B0"))+
  theme_bw()+ theme(legend.position="bottom")+ ptheme 
  ## SAVE GRAPH
    ggsave(p1,file=paste0(out1,"int_log_bars_dual.png"),width=10,height=7)

## COLORS USED IN SEFHIER PRESENTATION
c("darkslategray1", "darkslategray4","skyblue1","#74A9CF", "#0570B0","deepskyblue", "deepskyblue4")
...
## THE DENOMINATOR)
## BY STATE  
cnts_st<- cnts_st %>% mutate(ref_perc=100*(tot_refs/tot_ints),
             com_dec_perc=100*(tot_comp/tot_decs),
             com_log_perc=100*(tot_comp/tot_logs),
             int_dec_perc=100*(tot_ints/tot_decs),
             int_log_perc=100*(tot_ints/tot_logs))
...
p1<-ggplot(data=subset(cnts_st,!is.na(State)),aes(x=State,y=com_log_perc, group=State, fill=State)) +
    ...
    theme_bw()+ ptheme 
   ## SAVE GRAPH
   ggsave(p1,file=paste0(out1,"int_state.png"),width=10,height=7)
...
## THE DENOMINATOR)
## BY YEAR 
cnts_y<- cnts_y %>% mutate(ref_perc=100*(tot_refs/tot_ints),
            com_dec_perc=100*(tot_comp/tot_decs),
            com_log_perc=100*(tot_comp/tot_logs),
            int_dec_perc=100*(tot_ints/tot_decs),
            int_log_perc=100*(tot_ints/tot_logs))
...
tab_val<-c(cnts_y$tot_refs,cnts_y$tot_comp,cnts_y$tot_ints,cnts_y$ref_perc,cnts_y$com_log_perc)
tab_names<-c("Total Refusals","Completed Interviews","Total Interviews","Overall Refusal Rate","Overall Interception Rate")
## FINAL TABLE
...
piv_all3<- pivot_wider(int_cnts_stym,names_from = State,
