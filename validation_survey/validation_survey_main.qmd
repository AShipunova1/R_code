...
```{r the interviews by time window}
## the interviews by time window ----
```
...
```{r Interviews w no logbooks by captain name}
# Interviews w no logbooks by captain name ----
```
...
```{r 1 get all id_codes with no logbooks from the df joined by vessel and day and 2 get captain last name for these id_codes}
#### 1) get all id_codes with no logbooks from the df joined by vessel and day and 2) get captain last name for these id_codes ----
...
```{r 3 join logbooks to surveys which marked as having no logbooks to logbooks by date and captain last name}
#### 3) join logbooks to surveys which marked as having no logbooks to logbooks by date and captain last name ----
by_fields =
...
```{r check vessel ids in join_by_date_captain__has_lgb}
#### check vessel ids in join_by_date_captain__has_lgb ----
...
```{r write out check manually in PIMS add notes load back}
##### write out, check manually in PIMS, add notes, load back ----
# join_by_date_captain__has_lgb_short |>
...
```{r count interviews w no logbooks and checked captain names}
# count interviews w no logbooks and checked captain names ----
```

### remove same_vsls from interview_no_lgb 

```{r remove same_vsls from interview_no_lgb}
## remove same_vsls from interview_no_lgb ----
# <<<<
...
# 
# It returns the resulting data frame, which provides a summary of the counts of unique values for each column in the input data frame.
#  
#  
# >>>>
...
```{r keep only interview_no_lgb with no same captain or owner}
## keep only interview_no_lgb with no same captain or owner ----
...
```{r add fips for no cptn restored states}
## add fips for no cptn restored states ----
...
```{r counts for restored no cptn name}
### counts for restored no cptn name ----
...
```{r percent of interviews no lgb checked by captain name}
## percent of interviews no lgb, checked by captain name ----
...
```{r prep intv_w_no_lgb_join_by_day_vsl__minus_same_cptn_cnt for plotting}
## prep intv_w_no_lgb_join_by_day_vsl__minus_same_cptn_cnt for plotting  ----
...
```{r compare DISTANCE_CODE_NAME fishing_distance}
# compare DISTANCE_CODE_NAME fishing_distance ----
...
```{r compare CATCH_SPECIES_ITIS tsn}
# compare CATCH_SPECIES_ITIS tsn ----
```
