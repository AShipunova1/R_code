...
my_compliance_date_beg1 <- curr_dates[["my_compliance_date_beg"]]
str(my_compliance_date_beg1)
week(my_compliance_date_beg1)
# 52

wday(my_compliance_date_beg1,
     label = TRUE,
     week_start = week_start_day)
# Mon

# The end date
my_compliance_date_end1 <- curr_dates$my_compliance_date_end

# check
my_compliance_date_end1
# [1] "2023-12-31"

week(my_compliance_date_end1)
# 53

wday(my_compliance_date_end1,
     label = TRUE,
     week_start = week_start_day)
# Sun
...
late_submission_filter_stats <-
  function(my_df) {
    ...
    late_submission <-
      my_df |>
      filter(IS_MORE_THAN_30_DAYS_LATE == FALSE)
