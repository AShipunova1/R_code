...
```{r bad vs. total by diff for both}
### bad vs. total by diff for both ----
...
both_tot_w_coords__and_good_pairs_mark_cnts_wide_groups <- 
  ...
  mutate(my_group_b = base::findInterval(cnt_diff_b, c(0, 40)))
...
both_tot_w_coords__and_good_pairs_mark_cnts_wide_groups |>
  ...
  # geom_smooth(method = "lm", se = FALSE) +
  theme_bw() +
  labs(title = "Correlation between logbooks with wrong coordinates and\ntotal logbooks (w & w/o coord errors) divided by difference <= or > 35 (~mean)",
...
```{r plot bad vs. total for both cat low # no groups}
### plot bad vs. total for both cat low # no groups----
both_tot_w_coords__and_good_pairs_mark_cnts_wide_groups |>
  filter(wrong > 35) |> 
  ggplot(aes(y = total_trips_by_vsl,
...
```{r plot bad vs. total for both cat low # no groups}
lm.tw <- lm(both_tot_w_coords__and_good_pairs_mark_cnts_wide_groups$total_trips_by_vsl ~ both_tot_w_coords__and_good_pairs_mark_cnts_wide_groups$wrong)
