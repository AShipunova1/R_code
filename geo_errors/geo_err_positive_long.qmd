...
both_tot_w_coords__and_good_pairs_mark_cnts_wide_groups <- 
  ...
  mutate(my_group_b = base::findInterval(cnt_diff_b, c(0, group_boundary_both)))
...
p.smoothedmaxes <-
  ggplot_build(plot_both_tot_w_coords__and_good_pairs_mark_cnts_wide_groups_low)$data[[3]] %>%
  group_by(group) %>%
  mutate(xmean = mean(x)) %>%
  filter(x == max(x))
