...
coordInfo_summ2_short <-
  coordInfo_summ2 |>
  filter(is.na(mode)) |>
  dplyr::select(field_name, min, max, na_s) |>
  dplyr::distinct()
...
# trip_coord_info_vendors |>
#   dplyr::select(vendor_trip, vendor_effort) |>
#   dplyr::distinct() |>
#   dim()
# [1] 971   2
...
trip_coord_info_vendors3 |>
  dplyr::select(LATITUDE, LONGITUDE, vendor_trip_cat, year_start) |>
...
positive_long_vesl_cnts_short <-
  positive_long_vesl_cnts |>
  dplyr::select(VESSEL_ID, vesl_bad_vsl_cnt) |>
  dplyr::distinct()

all_vesl_cnts_short <-
  all_vesl_cnts |>
  dplyr::select(VESSEL_ID, all_bad_vsl_cnt) |>
  dplyr::distinct()
...
## r get Coastal_County_Divisions_for_Shoreline_Cleanup_Operations ----

path_to_coastal <-
  file.path(
            r"(C:\Users\anna.shipunova\Documents\R_files_local\my_inputs\shapefiles\Coastal_County_Divisions_for_Shoreline_Cleanup_Operations\Coastal_County_Divisions_for_Shoreline_Cleanup_Operations.shp)")
# "C:\Users\anna.shipunova\Documents\R_files_local\my_inputs\shapefiles\Coastal_County_Divisions_for_Shoreline_Cleanup_Operations\Coastal_County_Divisions_for_Shoreline_Cleanup_Operations.shp"

file.exists(path_to_coastal)

tic("coastal_sf")
coastal_sf <-
  sf::read_sf(path_to_coastal)
toc(log = TRUE, quiet = TRUE)
...
# print_df_names(coastal_df)
coastal_df$STATE |>
  unique() |>
  cat(sep = ", ")
# TX, LA, AL, FL, GA, SC, Puerto Rico, Virgin Islands, MS, MA
...
tic("us_maritime_sf")
us_maritime_sf <-
  sf::read_sf(path_to_us_maritime)
toc(log = TRUE, quiet = TRUE)
...
mapview(us_maritime_sf_east)

tic("sf::st_transform us_maritime_sf_east_trans")
us_maritime_sf_east_trans <-
  sf::st_transform(us_maritime_sf_east,
                   crs =
                     us_bb_border_crs) # Match the crs
toc(log = TRUE, quiet = TRUE)
...
res <-
  ...
  filter(lengths(
    sf::st_intersects(trip_coord_info_crop_big_sf_trans,
                      us_maritime_sf_east_trans)
  ) > 0)
# dim(res)
...
### r Make the same crs for us_bb_border and federal_state_w_sf_east ----
...
# 20
class(federal_state_w_sf_east_trans)
# sf, df



### r Make the same crs for us_bb_border and trip_coord_info_crop_big_sf ----
...
str(minu1)
# 0
mapview(federal_state_w_sf_east_trans) +
  mapview(us_bb_border_sf)
...
### r trips and state intersection ----

# mapview(trip_coord_info_crop_big_sf)
tic("trip_coord_info_sf_instate")
trip_coord_info_sf_instate <-
  trip_coord_info_crop_big_sf_trans |>
  filter(lengths(
    sf::st_intersects(trip_coord_info_crop_big_sf_trans,
                      federal_state_w_sf_east_trans)
...
tic.clearlog()
# trip_coord_info_sf_instate: 1.03 sec elapsed
mapview(trip_coord_info_sf_instate)
...
minus_sa_gom <-
  # trip_coord_info_crop_big_sf |>
  sf::st_filter(minus_sa,
            sf::st_union(in_gom), .predicate = sf::st_disjoint)
toc(log = TRUE, quiet = TRUE)
...
#
# minus_gom <- sf::st_difference(corrected_data_sf, gom_shp)

dim(minus_sa)
# [1] 44023    20

# minus_sa_gom <- sf::st_difference(gom_shp, minus_sa)
minus_sa_gom2 <- sf::st_difference(minus_sa, gom_shp)
...
# sf::st_difference(corrected_data_short_sf,
#               sf::st_union(sf::st_combine(c(gom_shp, sa_shp))))

union_shp <- sf::st_union(gom_shp, sa_shp)
# although coordinates are longitude/latitude, sf::st_union assumes that they are planar
# Warning message:
# attribute variables are assumed to be spatially constant throughout all geometries
...
corrected_data_short_minus_union_shp <-
  sf::st_difference(corrected_data_short_sf, union_shp)
# although coordinates are longitude/latitude, sf::st_difference
...
all_minus_sa <-
  sf::st_difference(corrected_data_short_minus_union_shp, sa_shp)
...
on_land <- sf::st_intersection(corrected_data_short_sf, fl_shp)
dim(on_land)
# [1] 3660   22
...
## r GOMsf_1 and us_bb intersection ----
ggplot() +
  ggplot2::geom_sf(data = sf::st_waters,
                   color = "red")
...
## r map_data intersection with points ----
...
trip_coord_info_map_data_sf_inland <-
  trip_coord_info_map_data_sf |>
  filter(lengths(
    sf::st_intersects(trip_coord_info_map_data_sf, polygon_usa)
  ) > 0)
...
### plot trip_coord_info_map_data_sf_inland ----
  ggplot() +
  geom_sf(
    data = trip_coord_info_map_data_sf_inland,
    fill = NA,
    color = "green"
  ) +
  coord_fixed(1.3)
