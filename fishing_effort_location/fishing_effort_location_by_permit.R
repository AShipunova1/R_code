...
effort_vsl_cropped_cnt_stat_zone <-
  effort_vsl_cropped |>
  # sf::st_drop_geometry() |>
  group_by(StatZone) |>
...
# check if there are xzzones with fewer than 3 vessels
effort_vsl_cropped_cnt_stat_zone |>
...
effort_vsl_cropped_cnt_stat_zone_short <-
  effort_vsl_cropped_cnt_stat_zone |>
  select(-c(LATITUDE, LONGITUDE, TRIP_ID, VESSEL_OFFICIAL_NBR))
...
map_trips_stat_zone <-
  make_map_trips(heat.plt_zone,
           GOMsf,
           "total trips",
           "trip_id_cnt_stat_zone",
           caption_text = "Heat map of SEFHIER trips. 2022. GoM permitted vessels. Only areas with more than 3 reporting vessels are shown. ")

map_trips_stat_zone

glimpse(heat.plt_zone)
# [1] "geometry, cell_id, StatZone, vsl_cnt_stat_zone, trip_id_cnt_stat_zone, x"

heat.plt_zone_zones_only_df <-
  heat.plt_zone |>
  sf::st_drop_geometry() |>
  select(-cell_id) |>
  as.data.frame()
...
ggplot() +
  geom_sf(data = heat.plt_zone_zones_only_df,
          aes(geometry = x,
              # aes(geometry = x,
              fill = trip_id_cnt_stat_zone,
          colour = "blue"))
