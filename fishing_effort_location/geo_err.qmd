...
trip_coord_info_crop_big_sf <-
  sf::st_crop(trip_coord_info_sf,
              big_bounding_box)

not_in <-
  lengths(sf::st_intersects(trip_coord_info_sf, trip_coord_info_crop_big_sf)) > 0

ocean = !not_in

# length(ocean)
# dim(trip_coord_info_sf)

trip_coord_info_sf_out <-
  trip_coord_info_sf |>
  # filter(hwy > 16) %>% 
  filter(lengths(sf::st_intersects(trip_coord_info_sf, trip_coord_info_crop_big_sf)) == 0)
  
dim(trip_coord_info_sf_inside)
# [1] 125242     38
