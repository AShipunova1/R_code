...
```{r remove state zones from GOMsf}
# str(GOMsf)
tic("sf::st_union(GOMsf)")
GOMsf_u <- sf::st_union(GOMsf)
toc(log = TRUE, quiet = TRUE)
```
...
GOMsf_1 <-
  sf::st_transform(GOMsf,
                   crs =
                     sf::st_crs(us_bb_border)) # Match the crs
toc()
```{r make the same crs for us_bb_border and GOMsf, cache=TRUE}
...
```{r check if us_bb_border has state water GOMsf}
# tic("us and gom inters")
# GOMsf_u__us_bb <-
#   GOMsf_u_1 |>
#   filter(lengths(
#     sf::st_intersects(GOMsf_u_1, us_bb_border)
#   ) > 0)
#   # no applicable method for 'filter' applied to an object of class "c('sfc_MULTIPOLYGON', 'sfc')"
# 
# toc()
...
tic("GOMsf_1 and us_bb difference")
not_st_waters <-
...
# ggplot() +
#   ggplot2::geom_sf(data = us_bb_border) +
#   ggplot2::geom_sf(data = GOMsf,
#                    color = "blue") +
#   ggplot2::ggtitle("US map includes state waters")

# mapview(st_waters)
# empty

# mapview(us_bb_border) +
#   mapview(GOMsf,
#           col.regions = "red")
