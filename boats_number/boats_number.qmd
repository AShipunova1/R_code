...
```{r How many SEFHIER vessels start at a different location than they end by quarter}
# How many SEFHIER vessels start at a different location than they end by quarter ----
...
```{r count total vessels by quarter}
## count total vessels by quarter ----
...
```{r see total vessels by 2022 start trip quarters}
## see total vessels by 2022 start trip quarters ----
all_logbooks_db_data_2022_short_p_region_dates_short_diff_start_from_end_tot |>
...
```{r add a start_ and end_ port_reg columns}
## add a start_ and end_ port_reg columns ----
...
```{r Count vessels having both sa and gom port_markers to find the num of vessels who fish in both the Gulf and S Atl}
### Count vessels having both sa and gom port_markers to find the num of vessels who fish in both the Gulf and S Atl ----
```
...
```{r Count vessels with each GOM or SA trip start and end port region marker}
#### Count vessels with each GOM or SA trip start and end port region marker ----
```
...
```{r Count vessels with each GOM or SA trip start and end port region marker per vessel permit_region}
#### Count vessels with each GOM or SA trip start and end port region marker per vessel permit_region ----
...
```{r Trip end ports are in both regions}
## Trip end ports are in both regions ----
...
```{r How many vessels have end port in one or in both regions}
#### How many vessels have end port in one or in both regions ----
end_ports_region_cnt |>
  mutate(
...
```{r Trip start and end ports are in both regions count by vessel permit}
## Trip start and end ports are in both regions count by vessel permit ----
```
...
```{r check start_  and end_ ports_region_cnt_by_permit_r}
#### check start_  and end_ ports_region_cnt_by_permit_r ----
start_ports_region_cnt_by_permit_r |>
...
```{r Count multiple start regions by permit_region}
#### Count multiple start regions by permit_region ----
start_ports_region_cnt_by_permit_r |>
  # glimpse()
  dplyr::select(
...
```{r Count multiple end regions by permit_region}
#### Count multiple end regions by permit_region ----
end_ports_region_cnt_by_permit_r |>
  # glimpse()
  dplyr::select(
...
```{r Show Number of vessels with different end trip port and home port per permit region}
#### Show Number of vessels with different end trip port and home port per permit region ----
...
### Total number of vessels by 2022 start trip quarters
