...
glimpse() |>
knitr::kable(caption = "all_logbooks_db_data_2022_short_p_region_short")
# 2
...
```{r how many vessels have variable landing locations (i.e., in the winter they are in one state while in the summer they fish in another);}
# how many vessels have variable landing locations (i.e., in the winter they are in one state while in the summer they fish in another); ----
...
glimpse() |>
knitr::kable(caption = "all_logbooks_db_data_2022_short_p_region_short_all_port_names_by_vsl")
...
```{r by quarter, to start - for some seasonality.}
## by quarter, to start - for some seasonality. ----
```

#### trips and quarter fields 

```{r trips and quarter fields}
### trips and quarter fields ----
...
```{r lists of start and end ports by quarter}
### lists of start and end ports by quarter ----
group_by_vector <-
  c("vessel_id",
    "vessel_official_nbr",
...
```{r if a num of lists of ports by quarter grater than 1, then ports are different from Q to Q}
### if a num of lists of ports by quarter grater than 1, then ports are different from Q to Q ----
...
```{r different locations with a combine table}
## different locations with a combine table ----
...
```{r count same or diff by permit_region}
### count same or diff by permit_region ----
all_logbooks_db_data_2022_short_p_region_dates_trip_port_short3_wider_diff |>
...
```{r count vessels having both one_start_port_markers to find the # of vessels who fish in both the gulf and S Atl.}
### count vessels having both one_start_port_markers to find the # of vessels who fish in both the gulf and S Atl.  ----
```

##### rm start_port_reg and start_port_fl_reg 

```{r rm start_port_reg and start_port_fl_reg}
#### rm start_port_reg and start_port_fl_reg ----
...
```{r count multi start by vessel permit}
### count multi start by vessel permit ----
...
glimpse() |>
knitr::kable(caption = "join_vessel_and_trip")
# 6
