...
connect_to_secpr  <-  function () 
{
    ...
    con <- dbConnect(dbDriver("Oracle"), username = my_username, 
...
remove_empty_cols  <-  function (my_df) 
{
    ...
    select(my_df, where(not_all_na)) %>% return()
}
...
{
    res_df <- clean_headers(read.xlsx(file_path, sheet_n, startRow = start_row, 
        detectDates = TRUE, colNames = TRUE, sep.names = "_"))
    return(res_df)
}
...
{
    res <- ungroup(mutate(rowwise(my_compl_df), compliant_after_override = case_when(!!sym(compliance_col_name) %in% 
        c(0, "NO") & !!sym(overridden_col_name) %in% c(0, "NO") ~ 
        "no", !!sym(compliance_col_name) %in% c(1, "YES") ~ "yes", 
        !!sym(overridden_col_name) %in% c(1, "YES") ~ "yes", 
        is.na(!!sym(compliance_col_name)) ~ NA, .default = toString(!!sym(compliance_col_name)))))
    return(res)
}
...
{
    res <- flatten(list(sort(unique(str_trim(my_lists)))))
    return(res)
}
