...
if (!get_username() == "anna.shipunova") {
  ...
  corresp_full_path <-
# Use function prepare_csv_full_path defined above.
...
# Use function prepare_csv_full_path(all_csv_names_list[2:length defined above.
    prepare_csv_full_path(all_csv_names_list[2:length(all_csv_names_list)],
...
#  
#  
# - The `clean_weeks` function is designed to clean up the "week" column in a compliance dataframe.
# 
...
{
    temp_df <- my_df %>% tidyr::separate_wider_delim(week, ":", 
        names = c("week_num", "week_rest")) %>% tidyr::separate_wider_delim(week_rest, 
 ...
 change_to_dates  <-  function (my_df, field_name, date_format = "") 
{
    ...
    result_df <- dplyr::mutate(my_df, `:=`(!!new_field_name, 
        as.POSIXct(!!field_name, tryFormats = my_tryFormats, 
            format = date_format)))
    return(result_df)
}
# >>>>
...
```{r}
csvs_clean1 <- 
# Use function clean_all_csvs defined above.
...
corresp_contact_cnts_clean0 <- 
  csvs_clean1[[1]] |> 
# Use function corresp_cleaning defined above.
  corresp_cleaning()
...
# Use function compliance_cleaning defined above.
  compliance_cleaning()
```
    ...
    force_from_db = NULL) 
{
    ...
    if (file.exists(my_file_path) & is.null(force_from_db)) {
        ...
        function_message_print(c("File", my_file_path, "doesn't exists, pulling data from database.", 
            "Must be on VPN."))
        msg_text <- paste(lubridate::today(), "run for", basename(my_file_path))
        tictoc::tic(msg_text)
...
```{r}
# Use function connect_to_secpr defined above.
...
```{r}
db_participants_address <-
# Use function read_rds_or_run defined above.
  read_rds_or_run(
    db_participants_address_file_path,
    db_participants_address_query,
    db_participants_address_fun,
...
# Use function remove_empty_cols defined above.
  remove_empty_cols() |>
# Use function clean_headers defined above.
  clean_headers()
```

## Load data from the previous results of "egregious violators for investigation" 

```{r Load data from the previous results of egregious violators for investigation}
# Load data from the previous results of "egregious violators for investigation" ----
# <<<<
...
# Use function pretty_print defined above.
...
{
    res <- dplyr::ungroup(dplyr::mutate(dplyr::rowwise(my_compl_df), 
        compliant_after_override = dplyr::case_when(!!rlang::sym(compliance_col_name) %in% 
            c(0, "NO") & !!rlang::sym(overridden_col_name) %in% 
            c(0, "NO") ~ "no", !!rlang::sym(compliance_col_name) %in% 
            c(1, "YES") ~ "yes", !!rlang::sym(overridden_col_name) %in% 
            c(1, "YES") ~ "yes", is.na(!!rlang::sym(compliance_col_name)) ~ 
...
compl_clean_w_permit_exp_last_half_year__sa__short__comp_after_overr <-
  compl_clean_w_permit_exp_last_half_year__sa__short |>
# Use function add_compliant_after_override defined above.
...
fhier_addr_short_clean <-
  fhier_addr_short |>
# Use function clean_names_and_addresses defined above.
...
# 0
```
We want a result of 0, if the value is non-zero, investigate using the line of code below.
...
# Use function list_sort_uniq defined above.
                                    ~ list_sort_uniq(.)),
             .keep = "none" ) |>
      dplyr::ungroup() |>
...
From Carolyn’semail and spreadsheet:
...
```{r}
# Use function print_df_names defined above.
