...
## Filter for non compliant, use all compliance data
```{r filter for non compliant, use all compliance data }
...
```{r Get compliance information for only vessels which have more than n "NO REPORT". }
...
## Add a call filter
```{r Add a filter: If there was 1 call or 2 emails (out and in, bc they got the email, we shared the information and received a confirmation) with a direct communication. }
...
```{r Combine compliance information with filtered correspondence info by vesselofficialnumber }
