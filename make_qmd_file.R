...
source_path_var_names <- find_source_path_vars()
...
make_source_path <- function(dir_to_comb, source_path_var_names) {
  source_path_var_names |>
    map(\(source_path_var_name) {
      if (length(source_path_var_name) > 0) {
        one_f_name <- str_remove(source_path_var_name, "_path")
        file.path(dir_to_comb, paste0(one_f_name, ".R"))
      }
    })
}

make_source_path(dir_to_comb, source_path_var_names)
