...
# check
compl_clean_sa_vs_gom_m_int_tot %>%
  select(year_permit, total_vsl_y, permit_sa_gom) %>%
  unique()
#   year_permit  total_vsl_y permit_sa_gom
#   <chr>              <int> <chr>        
# 1 2023 sa_only         2082 sa_only      
# 2 2023 sa_dual          339 dual         
...
compl_clean_sa_vs_gom_m_int_c_cnt_tot_wide_long |> 
  filter(year_permit == "2023 sa_only") |> 
...
compl_clean_sa_vs_gom_m_int %>%
  dplyr::filter(year_permit == "2023 sa_only") %>%
...
# compl_clean_sa_vs_gom_m_int_tot_exp_y_short_wide_long_cnt %>%
#   # remove NAs
#   filter(year_permit == '2023 gom_dual' & perm_exp_y == 'expired') %>% View()
...
# test
count_weeks_per_vsl_permit_year_compl_p %>%
  filter(permit_sa_gom == "sa_only", year == "2023") %>%
